...
| `apps/captain-adel-ios/` | Captain Adel — the offline GACAR co-pilot app | `apps/captain-adel-ios/README.md`, `apps/captain-adel-ios/ROADMAP.md`, `apps/captain-adel-ios/TESTFLIGHT_READINESS.md` |
...
`apps/flygaca-ios/CLAUDE.md` still calling itself "ay2m/FlyGACA" and listing a family-repo table
...
- **No shared Swift code exists yet.** `apps/flygaca-ios/apple/FlyGACAKit`'s target graph
  ...
  consequences).
- **No macOS/Xcode toolchain is available in a typical Linux Claude Code session.** You can run
