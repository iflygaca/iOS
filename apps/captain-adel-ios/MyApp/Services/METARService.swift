...
final class METARService: ObservableObject {
    ...
    private func formatAltimeter(_ val: Double?) -> String {
        guard let raw = val else { return "1013 hPa / 29.92 inHg" }
        let qnh: Double
        let inHg: Double
        if raw < 100 {
            // Provided in inHg (e.g. 29.92)
            inHg = raw
            qnh = raw * 33.8639
        } else {
            // Provided in hPa / millibars (e.g. 1013.2)
            qnh = raw
            inHg = raw * 0.02953
        }
    ...
    static func calculateCrosswind(windSpeed: Double, windDirection: Double, runwayHeading: Double) -> (crosswind: Double, headwind: Double) {
        var diff = (windDirection - runwayHeading).truncatingRemainder(dividingBy: 360)
        if diff > 180 { diff -= 360 }
        if diff < -180 { diff += 360 }
        let angleRadians = diff * .pi / 180.0
        let crosswind = abs(windSpeed * sin(angleRadians))
        let headwind = windSpeed * cos(angleRadians)
        return (crosswind, headwind)
    }

    // MARK: - Complete 18-Airport Saudi Aerodrome Database (Offline Baseline)
    static let defaultSaudiAirports: [METARReport] = [
        ...
        METARReport(
            ...
            airportNameAr: "مطار الأمير محمد بن عبد العزيز (المدينة)",
            flightCategory: .vfr,
        ...
        METARReport(
            ...
            remarks: "High elevation aerodrome (6,858 ft MSL)",
            isLive: false,
            lastUpdated: nil
        ),
        METARReport(
            ...
            airportNameAr: "مطار الأمير عبد المجيد (العلا)",
            flightCategory: .vfr,
        ...
        METARReport(
            ...
            airportNameAr: "مطار الأمير سلطان بن عبد العزيز (تبوك)",
            flightCategory: .vfr,
        ...
        METARReport(
            ...
            airportNameAr: "مطار الأمير نايف بن عبد العزيز (القصيم)",
            flightCategory: .vfr,
        ...
        METARReport(
            ...
            airportNameAr: "مطار الأمير عبد المحسن (ينبع)",
            flightCategory: .vfr,
        ...
        METARReport(
            ...
            airportNameAr: "مطار الملك عبد الله (جازان)",
            flightCategory: .mvfr,
